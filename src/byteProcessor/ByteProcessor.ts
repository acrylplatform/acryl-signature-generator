...
import { ALIAS_VERSION, TRANSFER_ATTACHMENT_BYTE_LIMIT, ACRYL_BLOCKCHAIN_ID, ACRYL_ID } from '../constants';


// NOTE : Acryl asset ID in blockchain transactions equals to an empty string
function blockchainifyAssetId(assetId: string): string {
    if (!assetId) throw new Error('Asset ID should not be empty');
    return assetId === ACRYL_ID ? ACRYL_BLOCKCHAIN_ID : assetId;
}
...
export class AssetId extends ByteProcessor {
    public process(value: string) {
        ...
        // We must pass bytes of `[0]` for Acryl asset ID and bytes of `[1] + assetId` for other asset IDs
